...
export const GridColumns = ({columns = []}) => {
    ...
    */
    return columns?.map(({
        ...
        return <Column sortOrder={sortOrder} format={isMoney ? 'currency' : format} allowEditing={allowEditing}
                       dataType={type} visible={visible} dataField={dataField} key={dataField} caption={displayName}
                       editorOptions={{
                           ...editorOptions,
            ...
            {
                lookup &&
                <Lookup dataSource={lookup.dataSource} valueExpr={lookup.valueExpr} displayExpr={lookup.displayExpr}/>
            }
            {
                isRequired && <RequiredRule message={"¡El campo '" + displayName + "' es requerido!'"}/>
            }
            {
                isPhone && (<StringLengthRule min={10} max={10}
                                              message={"El campo '" + displayName + "' es un telefono celular invalido."}/>)
            }
            {
