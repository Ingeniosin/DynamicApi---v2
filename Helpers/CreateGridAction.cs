...
public class CreateGridAction : IActionService<CreateGridInput> {
    ...
    private readonly string TemplateDependencyGrid =
        File.ReadAllText(@"D:\Escritorio\Lib\DynamicApi\Helpers\TemplateDependencyGrid.txt");
    ...
    public async Task<object> OnQuery(CreateGridInput input, HttpContext httpContext) {
        ...
        var dependency = properties.FirstOrDefault(x =>
            x.Name.ToLower().Replace("Id", "").Equals(input.Dependency.ToLower().Replace("Id", "").Trim()));
    ...
    private static string BuildColumns(IEnumerable<PropertyInfo> properties, IReadOnlyCollection<Type> models) {
        return string.Join(", \n\t", properties.Select(x => {
            ...
            var isNullable = x.PropertyType.IsGenericType &&
                             x.PropertyType.GetGenericTypeDefinition() == typeof(Nullable<>) && !isBoolean;
            ...
            if(isVirtual) {
                ...
                if(relationRoute != null) {
                    ...
                    var display =
                        relationRoute.GetProperties().FirstOrDefault(y => y.PropertyType == typeof(string))?.Name ??
                        "Undefined";
            ...
            return
                $"{{dataField: '{dataField}', dataType: '{type}', caption: captions['{x.Name.ToCamelCase()}'], required: {nullableStr}{lookup}}}";
        }));
    }
