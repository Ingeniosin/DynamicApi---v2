...
public class CreationHandler<T, TDbContext> : EntityStateHandler<T, TDbContext> where TDbContext : DynamicContext where T : class {
    
    public CreationHandler(ListenerConfiguration configuration, T model, TDbContext context, ListenerService<T, TDbContext> service) : base(configuration, model, context, service) {
    }
